...
function renderizar() {
    ...
    quadroLista.innerHTML += Object.values(cadastrados).map(e =>
        `${e.name} <br /><br />`).join('')
}
...
function renderSorteio() {
    ...
    for (let i = 0; i < participantes.length; i++) {
        sorteados.push([participantes[i], participantes[(i != participantes.length - 1) ? i + 1 : 0]]);
    }
    console.log('sorteados', sorteados);
    listaOne.innerHTML = ''
    listaOne.innerHTML += sorteados.map(i => `<div class="name-2"><h3>${i.toString().replace(",", " ==> ")}</h3></div>`).join(' ')
}
